...
-(void)capture_ocr_rec:(id)sender
{
    ...
    {
        recdata[i*3]=data[i*4+2];
        recdata[i*3+1]=data[i*4+1];
        recdata[i*3+2]=data[i*4+0];
    }

    struct timeval start;
    struct timeval end;
    gettimeofday(&start,0);
    vector<string> rst = ocr_recognize(recdata, width, height, 3);
...
//
//        //读取图片数据
//        NSString * imgpath=[NSString stringWithFormat:@"%@/n18.jpg",[[NSBundle mainBundle] bundlePath]];
