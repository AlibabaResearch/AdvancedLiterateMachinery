...
class TrainingArguments(transformers.TrainingArguments):
    ...
    vision_tower_training_enable: bool = False
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        if getattr(trainer.args, "vision_tower_training_enable", False):
            ...
            if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
                if current_folder.startswith('checkpoint-'):
                    ...
                    torch.save(weight_to_save, os.path.join(vision_tower_folder, f'{current_folder}.bin'))
                else:
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
        ...
        list_data_dict = [d for d in list_data_dict if d.get("conversations") != []]
        random.shuffle(list_data_dict)
        rank0_print(f"{len(list_data_dict)} data loaded.")
...
def train():
    ...
    if model_args.vision_tower is not None:
        ...
        if not training_args.vision_tower_training_enable:
            model.get_model().initialize_proj_modules(
                    model_args=model_args
                )
        ...
        if training_args.vision_tower_training_enable:
            model.get_model().initialize_proj_modules(
                    model_args=model_args
                )
