...
def main(config_dict):
    ...
    # data preparation
    train_dataset, train_dataloader = default_data(
        config_dict, config_dict['train_data_path'], training=True, shuffle=True, distributed=True)
    ...
    if config_dict['pretrained_path'] is not None:
        start_epoch, start_iter = save_load_tool.load(config_dict['pretrained_path'], local_rank)
    """ start training """
    ...
    for epoch in range(start_epoch, config_dict['num_epochs']):
        train_dataloader.sampler.set_epoch(epoch) # to guarantee different shuffling orders across epochs
        for it, (images, img_masks, labels, lengths, words_raw) in enumerate(train_dataloader):
            ...
            # validation part
            if i % config_dict['show_interval'] == 0 and local_rank == 0:
                ...
                loss_avg.reset()
            if i % config_dict['test_interval'] == 0 and local_rank == 0:
                net.eval()
...
if __name__ == "__main__":
    ...
    main(config_dict)
    print(f'Local Rank {local_rank}: end the training!')
