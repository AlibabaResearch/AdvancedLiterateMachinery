...
class SroieDataset(Dataset):
    ...
    def __init__(self, dataset_name, transforms=None):
        ...
        if self.split == 'train':
            self.img_path = './data/text_spotting_datasets/sroie/data/imgs/train/'
        else:
            self.img_path = './data/text_spotting_datasets/sroie/sroie/task3-test_347p/imgs/'
        ...
        self.classes = ['company', 'address', 'date', 'total']
    ...
    def parse_pt_poly_rec(self, word, category):
        quad = word
        x1, x2, x3, x4 = int(quad['polygon']['ptList'][0]['x']), int(quad['polygon']['ptList'][1]['x']), int(quad['polygon']['ptList'][2]['x']), int(quad['polygon']['ptList'][3]['x'])
        y1, y2, y3, y4 = int(quad['polygon']['ptList'][0]['y']), int(quad['polygon']['ptList'][1]['y']), int(quad['polygon']['ptList'][2]['y']), int(quad['polygon']['ptList'][3]['y'])
    ...
    def __getitem__(self, index):
        ...
        label_path = img_path.replace('imgs', 'anns').replace('train', 'new_train').replace('.jpg', '.json')
        ...
        for key,value in gt.items():
            ...
            for word in value:
                # if word['is_key'] == 1:
                #     continue
                pt_class, bezier_pt, rec = self.parse_pt_poly_rec(word, key)
                pts_class.append(pt_class)
                tmp_pts_class.append(pt_class)
                bezier_pts.append(bezier_pt)
                recs.append(rec)
                
                tmp_x1 = min(tmp_x1, int(word['polygon']['ptList'][0]['x']))
                tmp_y1 = min(tmp_y1, int(word['polygon']['ptList'][0]['y']))
                tmp_x3 = max(tmp_x3, int(word['polygon']['ptList'][2]['x']))
                tmp_y3 = max(tmp_y3, int(word['polygon']['ptList'][2]['y']))
            ...
            instance_classes.append(self.class2index[key])
        ...
        if 'train' in self.dataset_name:
            ...
            for index in sorted_index:
                ...
                for i in range(start_index, end_index, 1):
                    new_pts.append(center_pts[i].reshape(-1))
                    new_pts.append(target['instance_classes'][index].unsqueeze(0))
