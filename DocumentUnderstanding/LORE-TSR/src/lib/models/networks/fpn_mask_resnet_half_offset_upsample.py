...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers, heads, head_conv, **kwargs):
        ...
        self.deconv_layers1 = nn.Upsample(scale_factor=2, mode='bilinear',align_corners=True)#self._make_deconv_layer(1, [256], [4],)
        self.deconv_layers2 = nn.Upsample(scale_factor=2, mode='bilinear',align_corners=True)#self._make_deconv_layer(1, [256], [4],)
        self.deconv_layers3 = nn.Upsample(scale_factor=2, mode='bilinear',align_corners=True)#self._make_deconv_layer(1, [256], [4],)
        self.deconv_layers4 = nn.Upsample(scale_factor=2, mode='bilinear',align_corners=True)#self._make_deconv_layer(1, [256], [4],)
    ...
    def save_map(self,x,name):
        path = '/home/rujiao.lrj/CenterNet_cell_Coord/src/save_map.txt'
        f = open(path,'a+')
        f.write('\n---------------%s-------------------\n'%name)
        shape = list(x.shape)
        if shape[0]>3:
    ...
    def forward(self, x):
        #np.save('/home/rujiao.lrj/CenterNet_cell_Coord/src/caffe/input.npy',x.cpu().numpy())
        ...
        #np.save('/home/rujiao.lrj/CenterNet_cell_Coord/src/caffe/x4_input.npy',x4.cpu().numpy()) 
        x3_ = self.deconv_layers1(x4)
        #np.save('/home/rujiao.lrj/CenterNet_cell_Coord/src/caffe/x4_output.npy',x3_.cpu().numpy())
        #self.save_map(x3_.cpu().numpy()[0],'deconv_layers1.0')
        x3_ = self.adaption3(x3) + x3_
        #self.save_map(x3_.cpu().numpy()[0],'res4b_')
      
        #np.save('/home/rujiao.lrj/CenterNet_cell_Coord/src/caffe/x3_input.npy',x3_.cpu().numpy())
        x2_ = self.deconv_layers2(x3_)
        #np.save('/home/rujiao.lrj/CenterNet_cell_Coord/src/caffe/x3_output.npy',x2_.cpu().numpy())
        #self.save_map(x2_.cpu().numpy()[0],'deconv_layers2.0')
        x2_ = self.adaption2(x2) + x2_
        #self.save_map(x2_.cpu().numpy()[0],'res3b_')
 
        #np.save('/home/rujiao.lrj/CenterNet_cell_Coord/src/caffe/x2_input.npy',x2_.cpu().numpy())
        x1_ = self.deconv_layers3(x2_)
        #np.save('/home/rujiao.lrj/CenterNet_cell_Coord/src/caffe/x2_output.npy',x1_.cpu().numpy())
...
def get_pose_net_fpn_mask_half_offset(num_layers, heads, head_conv):
