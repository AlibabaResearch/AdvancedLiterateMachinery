...
def convkx1(in_planes, out_planes, kernal=15, stride=1):
    return nn.Conv2d(in_planes, out_planes, kernel_size=(kernal,1), stride=stride, padding=0, bias=False)

def conv1xk(in_planes, out_planes, kernal=15, stride=1):
    return nn.Conv2d(in_planes, out_planes, kernel_size=(1,kernal), stride=stride, padding=0, bias=False)
...
def single_pad(x,ksize,stride=1,Pool=False):
    shape = x.shape
    n_in,w,h = shape[1],shape[2],shape[3]
    if h % stride == 0:
        pad_along_height = max(ksize[1] - stride, 0)
    else:
        pad_along_height = max(ksize[1] - (h % stride), 0)
    if w % stride == 0:
        pad_along_width = max(ksize[0] - stride, 0)
    else:
        pad_along_width = max(ksize[0] - (w % stride), 0)
    pad_bottom = pad_along_height // 2
    pad_top = pad_along_height - pad_bottom
    pad_right = pad_along_width // 2
    pad_left = pad_along_width - pad_right
...
class GCBlock(nn.Module):
    def __init__(self, in_planes, ratio=16):
        ...
        self.kernal = 15
        self.left_k1 = convkx1(in_planes, in_planes // ratio, self.kernal)
        self.left_1k = conv1xk(in_planes // ratio, in_planes, self.kernal)
        self.right_1k = conv1xk(in_planes, in_planes // ratio, self.kernal)
        self.right_k1 = convkx1(in_planes // ratio, in_planes, self.kernal)

    def forward(self, x):
        out_x_left = single_pad(x,(self.kernal,1))
        out_x_left = self.left_k1(out_x_left)
        out_x_left = single_pad(out_x_left,(1,self.kernal))
        out_x_left = self.left_1k(out_x_left)

        out_x_right = single_pad(x,(1,self.kernal))
        out_x_right = self.right_1k(out_x_right)
        out_x_right = single_pad(out_x_right,(self.kernal,1))
        out_x_right = self.right_k1(out_x_right)
        
        return out_x_left + out_x_right
...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers, heads, head_conv, **kwargs):
        ...
        self.adaptionWH = nn.Conv2d(8, 8, kernel_size=1, padding=0, bias=False)

        self.GCBlock = GCBlock(self.heads['wh'],1)
    ...
    def forward(self, x):
        ...
        #self.save_map(x0_,'adaptionU1')

        ret = {}
        for head in self.heads:
            if head == 'wh':
                x0_wh = self.__getattr__(head)(x0_)
                x0_wh = self.GCBlock(x0_wh)
                x0_wh = nn.ReLU(inplace=True)(x0_wh)
                ret[head] = self.adaptionWH(x0_wh)
            else:
...
def get_pose_net_fpn_mask_half_offset_gc(num_layers, heads, head_conv):
